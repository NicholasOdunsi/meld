...
): Pick<MemberRow, "state" | "stateVariant"> {
  if (invitation.accepted_at) {
    return { state: "Accepted", stateVariant: "success" };
  }
  if (invitation.revoked_at) {
    return { state: "Revoked", stateVariant: "neutral" };
  }
  if (new Date(invitation.expires_at).getTime() <= Date.now()) {
    return { state: "Expired", stateVariant: "error" };
  }
  if (invitation.delivery_status === "failed") {
    return { state: "Delivery failed", stateVariant: "error" };
  }
  if (invitation.delivery_status === "pending") {
    return { state: "Sending", stateVariant: "warning" };
  }
  return { state: "Invited", stateVariant: "warning" };
}

function invitationIsActive(invitation: InvitationRecord) {
  return (
...
export default async function MembersPage({
  ...
  const invitationRows: MemberRow[] = invitations.map((invitation) => {
    ...
    const isActive = invitationIsActive(invitation);
    ...
    return {
      ...
      role: "Invitee",
      state: state.state,
      stateVariant: state.stateVariant,
