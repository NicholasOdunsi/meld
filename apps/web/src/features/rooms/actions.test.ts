...
describe("staged room attachments", () => {
  ...
  it("resolves an empty browser MIME type from the file extension", async () => {
    ...
    await stageRoomAttachment(formData);
  ...
  it("stages an image with a provisional caption and returns a signed view", async () => {
    ...
    const result = await stageRoomAttachment(form);
  ...
  it("claims metadata, removes storage, and then deletes the claim", async () => {
    ...
    await discardStagedRoomAttachment({
      roomId: ROOM_ID,
...
describe("listRoomInviteCandidates", () => {
  ...
  it("maps workspace members from the RPC result", async () => {
    ...
    const result = await listRoomInviteCandidates(WORKSPACE_ID);
    ...
    expect(mocks.rpc).toHaveBeenCalledWith("list_workspace_members", {
  ...
  it("throws a generic error when the RPC fails", async () => {
    ...
    await expect(
      listRoomInviteCandidates(WORKSPACE_ID),
    ).rejects.toThrow("We could not load workspace members.");
  });
  ...
  it("maps workspace members from the fake-mode store in test mode", async () => {
    mocks.isRoomFakeEnabled.mockReturnValue(true);
    mocks.listFakeWorkspacePeople.mockResolvedValue({
      isAdmin: false,
      members: [
        {
          user_id: "10000000-0000-4000-8000-000000000002",
...
describe("createRoomFromBrief", () => {
  ...
  it("posts an @Product Agent opener with the brief linked when ready", async () => {
    ...
    // The sidebar room list lives in the workspace layout; without this the
  ...
  it("caps staged attachments at ten (MessageInputSchema.attachmentIds's .max(10)) and reports the overflow file as unattached", async () => {
    ...
    // "staged room attachments" describe's beforeEach mocks this.
    mocks.persistAttachmentUpload.mockImplementation(
      async ({ attachment }) => ({
        id: attachment.id,
        message_id: null,
