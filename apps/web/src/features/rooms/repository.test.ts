...
): Parameters<typeof mapRoomMessageRow>[0] {
  return {
...
describe("PRD provenance on a message row", () => {
  it("maps the frozen PRD context from an initial query row", () => {
    const message = mapRoomMessageRow(prdContextRow());
  ...
  it("carries the identical frozen PRD context over the raw Realtime INSERT path", () => {
    ...
    const queryShaped = mapRoomMessageRow(prdContextRow());
    const realtimeShaped = mapRoomMessageRow(
      prdContextRow({
  ...
  it.each([
    ...
    const message = mapRoomMessageRow(prdContextRow(overrides));
...
it("maps Product Agent provenance through listMessages", async () => {
  ...
  const messages = await createRoomRepository(supabase).listMessages(
...
it("falls back to the room's own created_at when it has no messages", async () => {
  ...
  const rooms = await createRoomRepository(supabase).listRooms(
    WORKSPACE_ID,
  );
...
it("scopes the room query to the workspace and surfaces a failure", async () => {
  const { supabase, from, eq } = stubRoomsQuery([]);

  await createRoomRepository(supabase).listRooms(WORKSPACE_ID);
  ...
  await expect(
    createRoomRepository(failing.supabase).listRooms(WORKSPACE_ID),
...
it("requires an authenticated client and posts through the atomic RPC", async () => {
  ...
  await createRoomRepository(supabase).postMessage({
  ...
  expect(rpc).toHaveBeenCalledWith("post_room_message", {
...
it("returns the existing message for an idempotent client ID retry", async () => {
  ...
  const result = await createRoomRepository(
    supabase,
  ).postMessage({
...
it("surfaces an atomic attachment-link failure as a failed post", async () => {
  ...
  await expect(
    createRoomRepository(supabase).postMessage({
      roomId: "10000000-0000-4000-8000-000000000001",
      clientId: "20000000-0000-4000-8000-000000000002",
      body: "",
      mentionedUserIds: [],
...
it.each([
  {
    ...
    run: (
      repository: ReturnType<typeof createRoomRepository>,
    ) =>
  ...
  async ({ run, expectedFallback }) => {
    ...
    await expect(
      run(createRoomRepository(supabase)),
...
describe("listAttachmentStoragePaths", () => {
  it("returns the storage path of every attachment in the room", async () => {
    ...
    const paths = await createRoomRepository(
      supabase,
  ...
  it("surfaces a friendly error when the query fails", async () => {
    ...
    await expect(
      createRoomRepository(supabase).listAttachmentStoragePaths(
        "room-1",
      ),
...
describe("removeParticipant", () => {
  ...
  it("deletes the selected room participant", async () => {
    ...
    await expect(
      createRoomRepository(supabase).removeParticipant({
        roomId: "room-1",
        userId: "user-2",
      }),
    ).resolves.toBeUndefined();
...
describe("deleteRoom", () => {
  ...
  it("broadcasts a room-deleted event before deleting the room when the caller is the owner", async () => {
    ...
    await expect(
      createRoomRepository(supabase).deleteRoom("room-1"),
    ).resolves.toBeUndefined();
