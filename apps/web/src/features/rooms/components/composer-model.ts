...
export type QueuedRoomAttachment = {
  id: string;
  file: File;
  previewUrl?: string;
};
...
export type StagedComposerAttachment =
  | (QueuedRoomAttachment & { status: "uploading" })
  | (QueuedRoomAttachment & {
      status: "failed";
      error: string;
    })
  | (QueuedRoomAttachment & {
      status: "uploaded";
      uploaded: RoomAttachmentView;
    })
  | (QueuedRoomAttachment & {
      status: "discarding";
      uploaded: RoomAttachmentView;
...
export type ReadyRoomComposerAttachment = Extract<
  StagedComposerAttachment,
  { status: "uploaded" }
>;

export type RoomComposerSubmission = {
  body: string;
  attachments: ReadyRoomComposerAttachment[];
...
): attachment is ReadyRoomComposerAttachment {
...
export function deriveMentionSubmission(
  value: string,
  options: readonly RoomMentionOption[],
) {
...
export function buildSerializedMentionLookup(
  options: readonly RoomMentionOption[],
): Map<string, RoomMentionOption> {
  const lookup = new Map<string, RoomMentionOption>();
...
export function validateQueuedFiles(
  current: readonly QueuedRoomAttachment[],
  incoming: readonly File[],
) {
  const accepted: QueuedRoomAttachment[] = [];
  const errors: string[] = [];
...
// another workspace. This validator is the single gate. It accepts ONLY a
// relative Room path inside the given workspace and rejects
// everything else -- absolute URLs, protocol-relative URLs, backslash tricks,
// encoded traversal, and another workspace's id.

const ROOM_PATH = /^\/[0-9a-fA-F-]{36}\/rooms\/[0-9a-fA-F-]{36}$/;
...
export function isValidRoomReturnPath(
  ...
  // The workspace segment must be exactly the current workspace: a
  // well-formed room path under a different workspace id is still an escape.
  return decoded.startsWith(`/${workspaceId}/rooms/`);
}

// Build the room path a returnTo should carry. Returns null (never an unsafe
// string) if the ids do not compose a valid in-workspace room path, so a
// caller can decline to navigate rather than emit an open redirect.
export function buildRoomReturnPath(
  workspaceId: string,
  roomId: string,
): string | null {
  const path = `/${workspaceId}/rooms/${roomId}`;
  return isValidRoomReturnPath(path, workspaceId) ? path : null;
}
