-- The materializer queues the next link in a chain.
--
-- Task 1 (202608250001) added chain state to design_screen_generations and
-- the trigger-safe queue_design_screen_chain_step(). This wires it into the
-- batch materializer: once a completed run's screens are written, work out
-- what the flow still needs and queue exactly one follow-up.
