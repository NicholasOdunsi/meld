-- PostgreSQL cannot alter a RETURNS TABLE row shape with create or replace,
-- so recreate the same participant-scoped function at the same input
-- signature. The authorization body, search_path, revokes, and grant remain
-- unchanged; only the safe kind column is added to the return projection.
