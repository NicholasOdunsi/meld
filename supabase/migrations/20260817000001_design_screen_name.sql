-- Persist the model-generated screen name to design_screens.name.
--
-- The connector now emits `result_json.payload.screens[].name` per screen.
-- This redefinition of the live `materialize_design_screen_generate()`
-- (verbatim from 202608150013_materialize_layouts.sql) makes two changes:
--
--   1. New-sibling INSERT: the row's `name` now prefers the model's
--      declared `name`, then falls back to the existing slug-from-key
--      derivation, then the numbered placeholder -- unchanged fallback
--      chain, just with the model name taking priority.
--   2. Target / existing screen: after that screen's version is written,
--      an UPDATE overwrites `name` with the model's declared name, but
--      ONLY when the screen's current name is still exactly the default
--      placeholder `'Screen'`. A hand-typed name or a flow-seeded name is
--      never touched -- the guard is an exact-match on the placeholder,
--      not a heuristic.
--
-- Everything else -- screen resolution, layout fan-out, versioning,
-- events, and the replay guard -- is carried over verbatim.
