-- A room can have at most one PRD generation that is still able to advance.
-- Terminal tasks remain as history and do not prevent an explicit recovery.
create unique index ai_tasks_one_active_prd_generate_per_room
  on public.ai_tasks (room_id)
  where kind = 'prd_generate'
    and status in ('queued', 'waiting_for_device', 'ready_to_run', 'running');

create or replace function public.create_prd_generate_task(
  target_room_id uuid,
...
begin
  ...
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(target_room_id::text, 0)
  );
