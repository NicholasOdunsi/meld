-- Asking to change a screen you had not selected built a second copy of it,
-- and that copy took the key -- leaving the original unreachable and silently
-- re-pointing every button at a version nobody asked for.
--
-- Proof from live data: two "Vehicle Detail" screens four minutes apart, the
-- first with its screen_key stripped, and eight buttons across Vehicle
-- Storefront and Checkout all resolving to the second.
--
-- The cause is that with nothing selected the originating screen is created
-- BEFORE the model runs, so it is an empty placeholder standing in for an
-- answer nobody has yet. idx > 0 already resolves by key; idx = 0 pinned to
-- the originating screen "regardless of what key it names itself", so Policy A
-- then had to strip the incumbent to keep the key unique.
--
-- Now a placeholder resolves by key too, and is retired once its version lands
-- on the screen that owns that key. A screen the person actually selected has
-- content, so it is never treated as a placeholder and still receives the
-- version whatever key is declared -- Policy A is untouched for it.
