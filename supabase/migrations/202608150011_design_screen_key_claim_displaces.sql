-- Policy A for screenKey collisions in the batch materializer.
--
-- Before: when the actively generated originating screen declared a key that a
-- *different* live screen already held, the materializer silently left the new
-- screen's `screen_key` NULL (202608150003_design_screen_batch_materialize.sql).
-- A null-keyed screen is unaddressable: no `targetScreenKey` can resolve to it,
-- so every button meant to reach it dead-ends and the screen is orphaned --
-- reachable outward, never back. Regeneration churn (generate a new dashboard
-- while the previous one is still live, then delete the old one) reliably
-- produced exactly this: the live dashboard keyless, and every "All Transfers"
-- button pointing at a `dashboard` key no live screen owned.
--
-- After (Policy A): the screen being generated claims the key it declares. If a
-- different live screen currently holds that key, the incumbent yields it (its
-- key is nulled) -- the actively generated screen is the one the user is
-- working on and must be addressable by the name it declares. The invariant
-- "at most one live screen per key" is preserved; the stale incumbent simply
-- becomes keyless (and is typically the copy being superseded).
--
-- Only the collision branch changes. Everything else -- idx=0 pinning, resolve-
-- or-create by key for idx>0, layout, versioning, events, replay guard -- is
-- carried over verbatim from 202608150003.
