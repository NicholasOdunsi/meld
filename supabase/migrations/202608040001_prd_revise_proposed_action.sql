-- Allow the Product Agent to propose revising an existing PRD, not only
-- generating a fresh one. The message proposed_action still must be exactly one
-- of the two canonical objects -- no extra keys, no other kinds.
alter table public.messages drop constraint messages_proposed_action_shape;

alter table public.messages
  add constraint messages_proposed_action_shape
  check (
...
-- Carry a validated prd_revise proposal from the settled task onto the
-- Product Agent message (mirrors the existing prd_generate mapping).
CREATE OR REPLACE FUNCTION public.settle_ai_task(target_task_id uuid, target_device_id uuid, target_attempt_id uuid, target_operation ai_task_settle_operation, target_code task_error_code, target_message text, target_result jsonb, target_partial boolean)
 RETURNS ai_task_status
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
AS $function$
  ...
  then
    ...
    then
      ...
      reply_proposed_action := case
        ...
        then jsonb_build_object('kind', 'prd_revise')
        else null
      end;
