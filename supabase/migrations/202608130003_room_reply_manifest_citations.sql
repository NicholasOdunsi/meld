-- Keep room-reply citation authorization identical to the connector. A reply
-- exposes only message/evidence citation arrays, but its frozen context may
-- contain messages, evidence, attachments, and decisions.
    ...
    then
      ...
      then
        ...
        if reply_valid then
          ...
          -- The reply schema has only message and evidence citation arrays,
          -- while frozen context also contains attachments and decisions. Both
          -- citation arrays therefore authorize against the complete frozen
          -- manifest, matching the connector's citesOnlyAuthorizedIds rule.
