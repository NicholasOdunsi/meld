...
select is(
  ...
  'RPC-created room',
  'workspace member can create a room through the database function'
);
...
select is(
  ...
  0,
  'non-participant workspace member cannot read the created room'
);
...
select lives_ok(
  ...
  $$,
  'owner can add a workspace member as an explicit participant'
);
...
select throws_ok(
  $$
    select public.link_staged_room_attachments(
      '30000000-0000-4000-8000-000000000001',
      '50000000-0000-4000-8000-000000000003',
      array[
        '60000000-0000-4000-8000-000000000004',
...
select throws_ok(
  $$
    select public.post_room_message(
      '30000000-0000-4000-8000-000000000001',
...
select is(
  (select count(*)::int from public.rooms),
  0,
  'workspace membership alone cannot select a room'
);

select is(
  (select count(*)::int from public.messages),
  0,
  'workspace membership alone cannot select room messages'
);
...
select throws_ok(
  ...
  '42501',
  null,
  'non-participant workspace member cannot post'
);
